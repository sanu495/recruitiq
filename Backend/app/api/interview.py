...
def schedule_interview(
    ...
    if application.stage not in ("applied", "screening", "interview"):
        raise HTTPException(
            status_code=400,
            detail=f"Cannot schedule interview for stage: {application.stage}"
        )

    existing = session.exec(
        select(InterviewSlot).where(
            InterviewSlot.application_id == data.application_id,
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="An interview is already scheduled for this candidate. Update or cancel it first."
        )
    ...
    notif_dal.create(Notification(
        ...
        message=(
            f"[{job.title}] Interview scheduled on "
            f"{data.scheduled_at.strftime('%d %b %Y at %H:%M')} "
            f"({data.duration_minutes} min). {details}"
        )
    ))
...
def list_interviews(
    ...
    if current_user.role in ("recruiter", "admin"):
        if current_user.role == "admin":
            slots = session.exec(
                select(InterviewSlot).order_by(InterviewSlot.scheduled_at)
            ).all()
        else:
            job_ids = [
                j.id for j in session.exec(
                    select(Job).where(Job.recruiter_id == current_user.id)
                ).all()
            ]
            app_ids = [
            ...
            slots = session.exec(
                ...
                .where(InterviewSlot.application_id.in_(app_ids))
                .order_by(InterviewSlot.scheduled_at)
            ).all() if app_ids else []
    else:
        app_ids = [
            a.id for a in session.exec(
    ...
    if current_user.role in ("recruiter", "admin"):
        if current_user.role == "admin":
            slots = session.exec(
                select(InterviewSlot)
                .where(
                    InterviewSlot.scheduled_at >= lookback,
                    InterviewSlot.status.in_(["scheduled", "confirmed"])
                )
                .order_by(InterviewSlot.scheduled_at)
            ).all()
        else:
            ...
            slots = session.exec(
                select(InterviewSlot)
                .where(
                    InterviewSlot.application_id.in_(app_ids),
                    InterviewSlot.scheduled_at >= lookback,
                    InterviewSlot.status.in_(["scheduled", "confirmed"])
                )
                .order_by(InterviewSlot.scheduled_at)
...
def update_interview(
    ...
    if data.scheduled_at:
        notif_dal.create(Notification(
            user_id=application.candidate_id,
            message=(
                f"[{job.title}] Your interview has been rescheduled to "
                f"{data.scheduled_at.strftime('%d %b %Y at %H:%M')}."
            )
        ))
    elif data.status:
...
def confirm_interview(
    ...
    if slot.status != "scheduled":
        raise HTTPException(
            status_code=400,
            detail=f"Cannot confirm interview with status: {slot.status}"
        )
    ...
    notif_dal.create(Notification(
        ...
        message=(
            f"{current_user.name} has confirmed their interview for "
            f"'{job.title}' on {slot.scheduled_at.strftime('%d %b %Y at %H:%M')}."
        )
    ))
...
def cancel_interview(
    ...
    if current_user.role == "candidate":
        notif_dal.create(Notification(
            user_id=slot.created_by,
            message=f"{current_user.name} has cancelled their interview for '{job.title}'."
        ))
    else:
        notif_dal.create(Notification(
            user_id=application.candidate_id,
            message=(
                f"[{job.title}] Your interview scheduled on "
                f"{slot.scheduled_at.strftime('%d %b %Y at %H:%M')} "
